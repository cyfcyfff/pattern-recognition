...
class LeNet(nn.Module):
    def __init__(self, class_nums_1=4, class_nums_2=3):
        ...
        self.conv = nn.Sequential(
            # 输入通道数为3，输出通道数为6，卷积核大小为5x5，步长为1的卷积层
            nn.Conv2d(in_channels=3, out_channels=6, kernel_size=5, stride=1),
            # 使用带有负斜率的ReLU激活函数
            nn.LeakyReLU(0.1),
            # 使用大小为2x2，步长为2的最大池化层进行下采样。
            nn.MaxPool2d(kernel_size=2, stride=2),
            # 使用输入通道数为6，输出通道数为16，卷积核大小为5x5，步长为1的卷积层
            nn.Conv2d(in_channels=6, out_channels=16, kernel_size=5, stride=1),
            # 使用带有负斜率的ReLU激活函数
            nn.LeakyReLU(0.1),
            # 大小为2x2，步长为2的最大池化层进行下采样
            nn.MaxPool2d(kernel_size=2, stride=2),
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),                      # ReLU激活函数
            nn.Linear(84, 10)             # 输入特征的大小为84，输出特征的大小为10的全连接层
        )
