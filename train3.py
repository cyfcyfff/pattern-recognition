    ...
    with torch.no_grad():  # 禁用梯度计算
        ...
        for rgb, label1, label2 in data_loader:
            ...
            for sample_id in range(label1.shape[0]):
                if label1[sample_id].item() == one_hot_to_label(y1[sample_id]) and label2[
                    sample_id].item() == one_hot_to_label(y2[sample_id]):
                    corrs += 1
        net.train()  # 切换回训练模式
...
def train(net, dataloader_train, dataloader_test, epochs, optimizer, scheduler=None, logger=None):
    ...
    for epoch in range(epochs):
        ...
        tqdm.write(
            f'epoch = {epoch + 1} training_accs = {training_accs}  testing_accs={testing_accs} loss = {loss.item()}')  # 命令行界面输出结果
...
if __name__ == '__main__':
    ...
    epoches = 200  # 模型训练的轮数
    learning_rate = 0.01  # 模型在训练过程中的学习速率
    # ============== End ===================
